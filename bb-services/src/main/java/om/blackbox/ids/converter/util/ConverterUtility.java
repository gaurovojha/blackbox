...
public class ConverterUtility {
	...
	public static String getConvertedValue(ConversionStrategy conversionStrategy, String originalValue, Calendar fillingDate){
		...
		case CORRECT_FORMAT:
			...
			break;
		case ADD_YEAR:
			convertedValue = addYear(originalValue, fillingDate);
			break;
		case ADD_WO:
	...
	private static String addYear(String originalValue, Calendar fillingDate){
		int century = (fillingDate.get(Calendar.YEAR) / 100);
		return new StringBuilder(originalValue).insert(6, century).toString();
	}
