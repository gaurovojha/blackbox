...
<input type="hidden" id="destListSize" value="${destListSize}"/>
					...
					<div class="switch-control on-off" data-toggle="tooltip" title="${familyDestdata.createdDate}" id="${familyDestdata.familyId}">
						<label class="switch"> 
							...
							<c:if test="${familyDestdata.status eq 'INACTIVE'}">
								...
								<input type="checkbox" class="switch-input" id="toggleCheck" checked="${status}" onclick="familySwitchData('${familyDestdata.targetApplication.dbId}')"> 
							</c:if>
							<c:if test="${familyDestdata.status eq 'ACTIVE'}">
								<input type="checkbox" class="switch-input" id="toggleCheck" onclick="familySwitchData('${familyDestdata.targetApplication.dbId}')"> 
							</c:if>
